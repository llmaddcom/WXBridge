"""
OpenAI ChatCompletion 适配器示例

演示如何接入 OpenAI API，实现多轮对话（每个微信用户独立对话历史）。

依赖：pip install openai

用法：
    export OPENAI_API_KEY=sk-...
    python examples/openai_adapter.py
"""
import asyncio
import logging
import os

from wxbridge import AIAdapter, WeixinBridge, WeixinMessage

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(name)s: %(message)s")


class OpenAIAdapter(AIAdapter):
    """
    OpenAI ChatCompletion 适配器

    以 from_user_id 为 key 维护每用户独立的对话历史。
    注意：历史存储在内存中，服务重启后清空。如需持久化，请将 _histories 接入数据库。
    """

    def __init__(
        self,
        api_key: str | None = None,
        model: str = "gpt-4o",
        system_prompt: str = "你是一个友好的 AI 助手。",
        max_history: int = 20,
    ) -> None:
        try:
            from openai import AsyncOpenAI
        except ImportError as exc:
            raise ImportError("OpenAIAdapter 需要安装 openai：pip install openai") from exc

        self._client = AsyncOpenAI(api_key=api_key or os.environ["OPENAI_API_KEY"])
        self._model = model
        self._system_prompt = system_prompt
        self._max_history = max_history
        # from_user_id → [{role, content}, ...]
        self._histories: dict[str, list[dict[str, str]]] = {}

    async def reply(self, message: WeixinMessage) -> str:
        uid = message.from_user_id
        history = self._histories.setdefault(uid, [])

        # 超出最大历史长度时截断（保留最新的 max_history 条）
        if len(history) >= self._max_history:
            history[:] = history[-self._max_history + 1:]

        history.append({"role": "user", "content": message.text or ""})

        messages = [{"role": "system", "content": self._system_prompt}] + history

        response = await self._client.chat.completions.create(
            model=self._model,
            messages=messages,  # type: ignore[arg-type]
        )

        answer = response.choices[0].message.content or ""
        history.append({"role": "assistant", "content": answer})
        return answer


async def main() -> None:
    bridge = WeixinBridge(
        adapter=OpenAIAdapter(model="gpt-4o"),
        redis_url="redis://localhost",
    )

    token_info = await bridge.auth.load_token()
    if not token_info:
        print("尚未登录，请先运行 echo_adapter.py 完成扫码登录")
        return

    await bridge.start()
    print(f"WXBridge OpenAI ({OpenAIAdapter.__name__}) 已启动，按 Ctrl+C 停止")

    try:
        await asyncio.Event().wait()
    except (KeyboardInterrupt, asyncio.CancelledError):
        pass
    finally:
        await bridge.stop()


if __name__ == "__main__":
    asyncio.run(main())
